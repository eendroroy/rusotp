...
fn generate_hotp_sha256(c: &mut Criterion) {
    let hotp = HOTP::new(Algorithm::SHA256, "12345678901234567890", 6, Radix(10)).unwrap();
...
fn generate_hotp_sha512(c: &mut Criterion) {
    let hotp = HOTP::new(Algorithm::SHA512, "12345678901234567890", 6, Radix(10)).unwrap();
...
fn verify_hotp_sha1_success(c: &mut Criterion) {
    let hotp = HOTP::new(Algorithm::SHA1, "12345678901234567890", 6, Radix(10)).unwrap();
